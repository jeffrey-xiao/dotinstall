...
class PromptTest(unittest.TestCase):
    ...
    def test_prompt_y(self):
        ...
        self.assertTrue(os.path.exists(expand("~/test1/test1")))
        self.assertTrue(os.path.exists(expand("~/test2/test2")))
        clean("~/test1")
        clean("~/test2")
    ...
    def test_prompt_n(self):
        with patch.object(builtins, 'input', return_value='n'):
            execute_main('prompt', prompt=True)
        self.assertFalse(os.path.exists(expand("~/test1/test1")))
        self.assertFalse(os.path.exists(expand("~/test2/test2")))
        clean("~/test1")
        clean("~/test2")
