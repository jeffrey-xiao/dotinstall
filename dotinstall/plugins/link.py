    ...
    def execute(self, options, data, pkgManager):
        ...
        for linkLocation in data['linkLocations']:
            for pattern, path in linkLocation.items():
                ...
                Logger.logPipe(subprocess.Popen(["mkdir", "-pv", location], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL))
                for filename in glob.iglob(os.path.join(options['src'], data['package'], pattern)):
                    ...
                    if data['overwrite']:
                        Logger.logPipe(subprocess.Popen(["rm", os.path.join(expandPath(location), basename)], stdout=subprocess.PIPE, stderr=subprcoess.DEVNULL))
