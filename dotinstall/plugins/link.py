        ...
        for link_location in data['linkLocations']:
            for pattern, dest_path in link_location.items():
                ...
                for filename in glob.iglob(os.path.join(options['src'], data['package'], pattern)):
                    ...
                    if data['overwrite']:
                        subprocess.call(["rm", os.path.join(path.expand_path(location), basename)], stderr=subprocess.DEVNULL)
                        subprocess.call(["ln", "-sfv", filename, path.expand_path(location)])
                    else:
                        subprocess.call(["ln", "-sv", filename, path.expand_path(location)])
